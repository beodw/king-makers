...
import { Campaign, CampaignsValidationObject, ChosenDate } from './types';

const App:React.FC = () => {
  let [campaigns, setCampaigns] = useState(sampleData)
  let [campaignIdSet, setCampaignIdSet] = useState(new Set<number>())
  useEffect(
    () => {
      for(let campaign of campaigns){
        campaignIdSet.add(campaign.id)
      }
      setCampaignIdSet(new Set<number>(campaignIdSet))
    }, []
  )
  ...
  const _addCampaigns = (newCampaigns:Array<Campaign>) => {
    // add new campaigns only if array of values are valid.
    const campaignsValidationObject:CampaignsValidationObject = newCampaignsIsValid(newCampaigns, campaignIdSet)
  ...
  const updateSelectedCampaignStartDate = (chosenDate:ChosenDate) => {
    setSelectedCampaignStartDate(chosenDate.startDate ?? "")
  }
  const updateSelectedCampaignEndDate = (chosenDate:ChosenDate) => {
